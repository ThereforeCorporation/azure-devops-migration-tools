            ...
            try
            {
                ...
                foreach (var sourceIteration in sourceIterations)
                {
                    try
                    {
                        ...
                        var targetIdenties = targetHttpClient.GetCapacitiesWithIdentityRefAsync(targetTeamContext, targetIteration.Id).ConfigureAwait(false).GetAwaiter().GetResult();
                        foreach(var targetIdentie in targetIdenties){
                            Log.LogInformation("targetDisplayName", targetIdentie.TeamMember.DisplayName.ToString());
                        }
                        ...
                        foreach (var sourceCapacity in sourceCapacities)
                        {
                            ...
                            Log.LogInformation("sourceDisplayName", sourceDisplayName.ToString());    
                            ...
                            _targetTeamFoundationIdentitiesLazyCache.Value.ForEach(i => Log.LogInformation("targetDisplayName", i.DisplayName));
